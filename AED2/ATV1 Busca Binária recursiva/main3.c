...
int encontrarElemento(int , int *, int, int);
...
int encontrarElemento(int num, int *vetor, int inicio, int fim){
  int meio;  
  ...
  if(vetor[meio] > num)   //o num é antes(menor) da posiçao do meio
    return encontrarElemento(num, vetor, inicio, meio-1);

  else   //o num é depois(maior) da posiçao do meio
    return encontrarElemento(num, vetor, meio+1, fim);
}

int main(void) {
  int tamanho, num, i, box;
  int* vetor = NULL;

  //printf("\ndigite a quantidade de elementos: ");
  scanf("%d", &tamanho);

  vetor = (int *)malloc(tamanho * sizeof(int));//criar um vetor com N casas vezes o tamanho de um float

  if (vetor == NULL  ){      //se der erro em criar o vetor isso avisara e sair do programa
    //printf("\nErro de alocacao de memoria");
    free(vetor);
    return NULL;
  }
  ...
  box = encontrarElemento(num, vetor, 0, tamanho-1);
