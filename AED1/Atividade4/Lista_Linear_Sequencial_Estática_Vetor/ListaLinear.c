  ...
  if (i > 0 && i <= 100) {
    ...
    for (cont[i - 1].numero = 0; j < 10 || j > 11;) { // salvar numero
      ...
      for (j = 0, box = cont[i - 1].numero; box != 0; j++) {
        // contar numeros digitados, condiçao nao importa por causa do break
        // abaixo
        box = box / 10;
      }

      if (j < 10 || j > 12) {
        // se diferente de 10 e 11 avise e repita novamente o codigo até estiver
        // certo
        printf("Telefone não tem 10 ou 11 dígitos! Favor digite novamente.\n");
      }
    }
    // reorganizar o numero guardado em p para o struct numero int, para poupar
    // espaço
...
void Apagar_contato_pelo_nome(contato *cont) { // opcao 5
  ...
  // se possuir um contato ele irá repetir 100 vezes o for do j mas quando
  // terminar o valor do j não será excluido, entao caso possuir o nome, o j terá
  // valor != 0 sempre
